...
void profile(void)
{
   ...
   FILE *fp;
   // openmp version
   char *version = "1.7.0 (modified w/OpenMP)";
   ...
   if (!my_pe) {
      ...
      if (report_perf & 4) {
         ...
         printf("\n ================== End report ===================\n");
printf("Summary: ranks %d threads %d ts %d time %lf calc %lf comm %lf red %lf refine %lf blocks/ts %lf max_blocks %d\n", num_pes, ompt, num_tsteps, average[0], average[38], average[37], average[39], average[42], ((double) total_blocks)/((double) (num_tsteps*stages_per_ts)), global_max_b);
fflush(NULL);
      }
   }
}
