...
void init(void)
{
   ...
   //printf("%i: num_vars = %i, sizeof(grid_sum)/sizeof(double) = %i\n", my_pe, num_vars, (int) sizeof(grid_sum)/sizeof(double));fflush(NULL);
   for (var = 0; var < num_vars; var++) {
      grid_sum[var] = check_sum(var);
      //printf("%i: grid_sum[%i] = %d", my_pe, var, grid_sum[var]);fflush(NULL);
   }
}
